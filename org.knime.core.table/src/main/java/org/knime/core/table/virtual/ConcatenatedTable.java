 ...
 */
class ConcatenatedTable implements RowAccessible {

    private final List<RowAccessible> m_delegateTables;

    private final ColumnarSchema m_schema;

    ConcatenatedTable(final List<RowAccessible> tablesToConcatenate, final ColumnarSchema concatenatedSchema) {
        m_delegateTables = tablesToConcatenate;
        m_schema = concatenatedSchema;
    }
    ...
    public Cursor<ReadAccessRow> createCursor() {
        return new ConcatenatedRowCursor(m_delegateTables, m_schema);
    }

    @Override
    public void close() throws IOException {
        for (int i = 0; i < m_delegateTables.size(); i++) {
            m_delegateTables.get(i).close();
        }
    }
