...
class RandomAccessNodeImpSource implements RandomAccessNodeImp {
    ...
    private RandomAccessCursor<ReadAccessRow> randomAccess;

    private LookaheadCursor<ReadAccessRow> lookahead;

    public RandomAccessNodeImpSource(RowAccessible accessible, int[] cols, long fromRow, long toRow) {
    ...
    public void create() {
        ...
        if (cursor instanceof RandomAccessCursor) {
            randomAccess = (RandomAccessCursor<ReadAccessRow>)cursor;
        }
    }

    @Override
    public void moveTo(long row) {
        randomAccess.moveTo(row);
    }
