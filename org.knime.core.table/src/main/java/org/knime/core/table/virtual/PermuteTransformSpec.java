...
public final class PermuteTransformSpec implements TableTransformSpec {
    ...
    private ColumnarSchema createSchema(final ColumnarSchema schema) {
        if (m_permutation.length != schema.numColumns()) {
            throw new IllegalArgumentException("Number of permutation indices and number of input columns differ: " +
                m_permutation.length + " vs " + schema.numColumns());
        }
        final DataSpec[] permutedSpecs = new DataSpec[m_permutation.length];
        for (int i = 0; i < m_permutation.length; i++) {
            permutedSpecs[i] = schema.getSpec(m_permutation[i]);
        }
    ...
    public List<RowAccessible> transformTables(final List<RowAccessible> tables) {
        ...
        return Arrays.asList(new PermutedTable(table, m_permutation, schema));
    }
    ...
    public static final class PermuteTransformSpecSerializer
        ...
        public PermuteTransformSpecSerializer() {
            super(PermuteTransformSpec.class, 0);
        }

        @Override
        public void write(final PermuteTransformSpec spec, final DataOutput output) throws IOException {
            SerializationUtil.writeIntArray(spec.m_permutation, output);
        }

        @Override
        public PermuteTransformSpec read(final DataInput input) throws IOException {
            return new PermuteTransformSpec(SerializationUtil.readIntArray(input));
        }
    }
}
