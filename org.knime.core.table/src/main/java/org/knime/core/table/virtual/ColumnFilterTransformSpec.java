...
public final class ColumnFilterTransformSpec implements TableTransformSpec {
    ...
    /**
     * @param columnIndices The indices of the columns in the original table to retain in the transformed table.
     */
    public ColumnFilterTransformSpec(final int[] columnIndices) {
        m_columnIndices = columnIndices.clone();
        int previousIndex = -1;
    ...
    private ColumnarSchema createSchema(final ColumnarSchema schema) {
        final DataSpec[] filteredSpecs = new DataSpec[m_columnIndices.length];
        for (int i = 0; i < m_columnIndices.length; i++) {
            filteredSpecs[i] = schema.getSpec(m_columnIndices[i]);
        }
    ...
    public List<RowAccessible> transformTables(final List<RowAccessible> tables) {
        ...
        return Arrays.asList(new ColumnFilteredTable(table, m_columnIndices, schema));
    }
