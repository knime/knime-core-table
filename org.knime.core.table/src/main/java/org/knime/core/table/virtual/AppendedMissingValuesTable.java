 ...
 */
class AppendedMissingValuesTable implements RowAccessible {

    private final RowAccessible m_delegateTable;

    private final ColumnarSchema m_schema;

    public AppendedMissingValuesTable(final RowAccessible tableToAppendTo, final ColumnarSchema appendedSchema) {
        m_delegateTable = tableToAppendTo;
        m_schema = appendedSchema;
    }
    ...
    private static final class AppendedMissingValuesCursor implements Cursor<ReadAccessRow> {
        ...
        private static final class AppendedMissingValuesReadAccessRow implements ReadAccessRow {
            ...
            public AppendedMissingValuesReadAccessRow(final ReadAccessRow delegateAccess, final ColumnarSchema schema) {
                ...
                m_schema = schema;
                m_numNonMissingColumns = m_delegateAccess.getNumColumns();
            ...
            public <A extends ReadAccess> A getAccess(final int index) {
                if (index < m_numNonMissingColumns) {
                    ...
                    final A casted = (A)m_missingColumnAccesses[index - m_delegateAccess.getNumColumns()];
                    return casted;
                }
            }
        }
