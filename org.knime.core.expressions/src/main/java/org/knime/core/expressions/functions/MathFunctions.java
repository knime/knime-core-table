...
public final class MathFunctions {
    ...
    public static final ExpressionFunction ROUNDHALFDOWN = functionBuilder() //
        ...
        .returnType("Nearest value to `x` with `precision` decimal places", RETURN_INTEGER_FLOAT_MISSING,
            args -> (args.length == 1) ? INTEGER(args[0].isOptional()) : FLOAT(args[0].isOptional())) //
    ...
    private static Function<List<Computer>, Computer> roundImplFactory(final RoundingMode mode,
        ...
        return args -> {
            ...
            if (args.size() == 1) {
                ...
                return FloatComputer.of(ctx -> {
                    var precisionArgument = args.get(1);

                    int scale = precisionArgument.isMissing(ctx) ? 0
                        : FunctionUtils.toIntExact(toInteger(args.get(1)).compute(ctx), "precision arg was too large.");
    ...
    public static final ExpressionFunction NORMAL = functionBuilder() //
        ...
        .description("""
                ...
                If the mean is `MISSING`, it will be treated as 0.
    ...
    private static Computer normalImpl(final List<Computer> args) {
        return FloatComputer.of(ctx -> {
            ...
            var mean = 0.0;
            if (!args.get(1).isMissing(ctx)) {
                mean = toFloat(args.get(1)).compute(ctx);
            }

            var standardDeviation = 1.0;
            if (args.size() > 2 && !args.get(2).isMissing(ctx)) {
                standardDeviation = toFloat(args.get(2)).compute(ctx);
    ...
    public static final ExpressionFunction ERROR_FUNCTION = functionBuilder() //
        ...
        .description("""
                ...
                specified mean and optionally the standard deviation.
