...
public final class DataTraitsSerializerTest<T extends DataTraits> {

    @Parameterized.Parameters(name = "{0}")
    public static Iterable<DataTraits> createDataSpecsToTest() {
        return Arrays.asList( //
            DefaultDataTraits.EMPTY, //
            new DefaultDataTraits(new LogicalTypeTrait("testType")), //
            new DefaultDataTraits(new DictEncodingTrait(KeyType.INT_KEY)), //
            new DefaultListDataTraits(new DataTrait[]{new DictEncodingTrait(KeyType.LONG_KEY)}, //
                new DefaultDataTraits(new LogicalTypeTrait("listTestType"))), //
            new DefaultStructDataTraits( //
                new DefaultDataTraits(new DictEncodingTrait(KeyType.BYTE_KEY)), //
                new DefaultDataTraits(new LogicalTypeTrait("innerTestType")) //
            ));
    }
    ...
    public void testDataSpecSerializationRoundtrip() {
        final JsonNode config = new DataTraitsSerializer(JsonNodeFactory.instance).save(m_traits);
        @SuppressWarnings("unchecked")
        final T deserialized = (T)DataTraitsSerializer.load(config);
        assertEquals(m_traits, deserialized);
    }
}
