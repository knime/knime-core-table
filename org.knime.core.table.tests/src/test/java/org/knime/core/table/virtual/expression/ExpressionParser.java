...
public class ExpressionParser implements PegParser<Expr>
{
    ...
    static _State match(int ruleId, _State state, int subIndex) throws _FatalEx
    {
        ...
        state = switch(ruleId){
            ...
            case 20 -> rule_20(state); // @Regex("[0-9]+")String
            case 21 -> rule_21(state); // @Ch("$")Void
            case 22 -> rule_22(state); // @Ch("[")Void
            case 23 -> rule_23(state); // String
            case 24 -> rule_24(state); // @Ch("]")Void
            case 25 -> rule_25(state); // @Ch(except="\\\" \t\n\r+-*/%",range={32,1114111})int[]
            case 26 -> rule_26(state); // @Ch("(")Void
            case 27 -> rule_27(state); // @Ch(")")Void
            case 28 -> rule_28(state); // @Ch("\"")Void
            case 29 -> rule_29(state); // int[]
            case 30 -> rule_30(state); // @Ch(except="\\\" \t\n\r+-*/%",range={32,1114111})int
            case 31 -> rule_31(state); // int
            case 32 -> rule_32(state); // int
            case 33 -> rule_33(state); // int
            case 34 -> rule_34(state); // int
            case 35 -> rule_35(state); // @Ch(except="\\\"",range={32,1114111})int
            case 36 -> rule_36(state); // @Ch("\\")Void
            case 37 -> rule_37(state); // @Ch("\\\"/bfnrt")char
            case 38 -> rule_38(state); // @Ch("u")Void
            case 39 -> rule_39(state); // byte
            case 40 -> rule_40(state); // @Regex("[0-9A-Fa-f]")char
    ...
    static _State rule_16(_State state) throws _FatalEx
    {
        ...
        state = match(20, state, 1); // arg_1: @Regex("[0-9]+")String
        if(state.fail)
            return state.fail(start0);
        String arg_1 = state.pickObj();
        ...
        Atom value;
        try{
            value = state.catalog.intconst(arg_0, arg_1, arg_2);
        }catch(Exception ex){
            throw new _FatalEx(start0, ex);
        }
        return state.ok(value);
    ...
    static _State rule_17(_State state) throws _FatalEx
    {
        ...
        state = match(21, state, 1); // arg_1: @Ch("$")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_1 = state.pickObj();

        state = match(22, state, 2); // arg_2: @Ch("[")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_2 = state.pickObj();

        state = match(23, state, 3); // arg_3: String
        if(state.fail)
            return state.fail(start0);
        String arg_3 = state.pickObj();

        state = match(24, state, 4); // arg_4: @Ch("]")Void
        if(state.fail)
            return state.fail(start0);
    ...
    static _State rule_18(_State state) throws _FatalEx
    {
        ...
        state = match(25, state, 2); // arg_2: @Ch(except="\\\" \t\n\r+-*/%",range={32,1114111})int[]
        if(state.fail)
            return state.fail(start0);
    ...
    static _State rule_19(_State state) throws _FatalEx
    {
        ...
        state = match(26, state, 1); // arg_1: @Ch("(")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_1 = state.pickObj();
        ...
        state = match(27, state, 3); // arg_3: @Ch(")")Void
        if(state.fail)
            return state.fail(start0);
    ...
    static final java.util.regex.Pattern pattern_20 = java.util.regex.Pattern.compile("[0-9]+", 0);

    // regex rule for: @Ch("$")Void
    static _State rule_21(_State state)
    {
        return match_regex(state, 21, pattern_21, 0);
    }
    ...
    // concat rule for: String
    static _State rule_23(_State state) throws _FatalEx
    {
        final int start0 = state.start;

        state = match(28, state, 0); // arg_0: @Ch("\"")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_0 = state.pickObj();

        state = match(29, state, 1); // arg_1: int[]
        if(state.fail)
            return state.fail(start0);
        int[] arg_1 = state.pickObj();

        state = match(28, state, 2); // arg_2: @Ch("\"")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_2 = state.pickObj();
    ...
    {
        state = match_repeat(state, 25, 30, 0, Integer.MAX_VALUE);
        if(state.fail)
            return state;
    ...
    {
        return match_regex(state, 26, pattern_26, 0);
    }
    static final java.util.regex.Pattern pattern_26 = java.util.regex.Pattern.compile("\\(", 0);
    ...
    {
        state = match_repeat(state, 29, 31, 0, Integer.MAX_VALUE);
        if(state.fail)
            return state;
        java.util.ArrayList<Integer> list = state.pickObj();
        int[] array = new int[list.size()];
    ...
    {
        return match_regex_int(state, 30, pattern_30, 0);
    }
    static final java.util.regex.Pattern pattern_30 = java.util.regex.Pattern.compile("[\\ -\\x{10FFFF}&&[^\\\\\"\\ \\t\\n\\r\\+\\-\\*/%]]", 0);

    // alt rule for: int
    static _State rule_31(_State state) throws _FatalEx
    {
        state = match(32, state, 0); // int
        if(!state.fail)
            return state;

        state = match(33, state, 1); // int
        if(!state.fail)
            return state;

        state = match(34, state, 2); // int
        if(!state.fail)
            return state;
    ...
    static _State rule_32(_State state) throws _FatalEx
    {
        ...
        state = match(35, state, 0); // arg_0: @Ch(except="\\\"",range={32,1114111})int
        if(state.fail)
            return state.fail(start0);
    ...
    static _State rule_33(_State state) throws _FatalEx
    {
        ...
        state = match(36, state, 0); // arg_0: @Ch("\\")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_0 = state.pickObj();

        state = match(37, state, 1); // arg_1: @Ch("\\\"/bfnrt")char
        if(state.fail)
            return state.fail(start0);
    ...
    static _State rule_34(_State state) throws _FatalEx
    {
        ...
        state = match(38, state, 1); // arg_1: @Ch("u")Void
        if(state.fail)
            return state.fail(start0);
        Void arg_1 = state.pickObj();

        state = match(39, state, 2); // arg_2: byte
        if(state.fail)
            return state.fail(start0);
        byte arg_2 = state.pickObj();

        state = match(39, state, 3); // arg_3: byte
        if(state.fail)
            return state.fail(start0);
        byte arg_3 = state.pickObj();

        state = match(39, state, 4); // arg_4: byte
        if(state.fail)
            return state.fail(start0);
        byte arg_4 = state.pickObj();

        state = match(39, state, 5); // arg_5: byte
        if(state.fail)
            return state.fail(start0);
    ...
    {
        return match_regex_int(state, 35, pattern_35, 0);
    }
    static final java.util.regex.Pattern pattern_35 = java.util.regex.Pattern.compile("[\\ -\\x{10FFFF}&&[^\\\\\"]]", 0);

    // regex rule for: @Ch("\\")Void
    static _State rule_36(_State state)
    {
        return match_regex(state, 36, pattern_36, 0);
    }
    ...
    static final java.util.regex.Pattern pattern_37 = java.util.regex.Pattern.compile("[\\\\\"/bfnrt]", 0);
    ...
    static final java.util.regex.Pattern pattern_38 = java.util.regex.Pattern.compile("u", 0);
    ...
    static _State rule_39(_State state) throws _FatalEx
    {
        ...
        state = match(40, state, 0); // arg_0: @Regex("[0-9A-Fa-f]")char
        if(state.fail)
            return state.fail(start0);
    ...
    {
        return match_regex_char(state, 40, pattern_40, 0);
    }
    static final java.util.regex.Pattern pattern_40 = java.util.regex.Pattern.compile("[0-9A-Fa-f]", 0);
    ...
    static class _DatatypeList
    {
        ...
        // [20] @Regex("[0-9]+")String
        static java.lang.@org.rekex.spec.Regex(flags=0, group=0, value="[0-9]+")String t_20;
        // [21] @Ch("$")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="$")Void t_21;
        // [22] @Ch("[")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="[")Void t_22;
        // [23] String
        static java.lang.String t_23;
        // [24] @Ch("]")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="]")Void t_24;
        // [25] @Ch(except="\\\" \t\n\r+-*/%",range={32,1114111})int[]
        static @org.rekex.helper.anno.Ch(except="\\\" \t\n\r+-*/%", ignoreCase=false, range={32, 1114111}, value="")int[] t_25;
        // [26] @Ch("(")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="(")Void t_26;
        // [27] @Ch(")")Void
        ...
        static int[] t_29;
        // [30] @Ch(except="\\\" \t\n\r+-*/%",range={32,1114111})int
        static @org.rekex.helper.anno.Ch(except="\\\" \t\n\r+-*/%", ignoreCase=false, range={32, 1114111}, value="")int t_30;
        // [31] int
        static int t_31;
        // [32] int
        static int t_32;
        ...
        // [34] int
        static int t_34;
        // [35] @Ch(except="\\\"",range={32,1114111})int
        static @org.rekex.helper.anno.Ch(except="\\\"", ignoreCase=false, range={32, 1114111}, value="")int t_35;
        // [36] @Ch("\\")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="\\")Void t_36;
        // [37] @Ch("\\\"/bfnrt")char
        static @org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="\\\"/bfnrt")char t_37;
        // [38] @Ch("u")Void
        static java.lang.@org.rekex.helper.anno.Ch(except="", ignoreCase=false, range={}, value="u")Void t_38;
        // [39] byte
        static byte t_39;
        // [40] @Regex("[0-9A-Fa-f]")char
        static @org.rekex.spec.Regex(flags=0, group=0, value="[0-9A-Fa-f]")char t_40;
